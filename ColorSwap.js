let userlv=localStorage.getItem("userlevel")||0

if (userlv==1) {startmaingame()}
   else{starttitorial()}

function startmaingame() {

let gameovereds=false
let score= 0;
...
function barcolor() {
          //if (barchangenum==0) {barchangenum++}
             ...
             //else if (barchangenum==3) {barchange3.play(),barchangenum=1}
if (barcount==1) {
smgs.innerText="Well Done!! Now Click Again to Change it to Black"
}
else if (barcount==2){
   smgs.innerText="Be Careful Now! Some Ball Will Start to Come Now!"
  setTimeout(()=>{smgs.innerText="You Need to Match The Color of Bar and Ball"},2500)
  setTimeout(()=>{barcolorchangeend=true},3500)
  setTimeout(()=>{smgs.innerText="You Are Doing Pretty Well! Keep Going!"},8500)
  setTimeout(()=>{smgs.innerText="Just a Few More to Finish The Tutorial"},15000)
  setTimeout(()=>{smgs.innerText="Great Job! You Almost Finished it! After This You Will be Ready to Play for Leaderboard"},20000)
  setTimeout(()=>{smgs.innerText="Click 'Start Real Game' to Finish it!"
                  barcolorchangeend=false},28000)
  setTimeout(()=>{realgame()},30000)

  function realgame() {
     let realgamebox=document.createElement("div")
     realgamebox.className="realgame"
     realgamebox.innerText="Start Real Game"
     realgamebox.addEventListener("click",endtitorial)

     document.body.appendChild(realgamebox)
  }

  function endtitorial() {
      ...
      setTimeout(()=>{
         ...
         textscore.style.animation="none"
           ...
           if (score<10) {score+=1}
                        ...
                        ball.style.animation="endpop 0.2s forwards"
                        bar.style.animation="endpop 0.2s forwards"
                       if (!gameovered) {setTimeout(()=>{gameover()},2)} 
         }
   }
...
function gameover() {
  if (!gameovered) { 
   ...
   let smgs=document.createElement("span")
   smgs.className="smgs"
   smgs.innerText="Tap in The Screen or Click Space to Change The Color of The Bar"

   document.body.appendChild(smgs)


   let skiptitorial=document.createElement("button")
   skiptitorial.className="skiptitorial"
   skiptitorial.innerText="Skip Titorial"
   skiptitorial.addEventListener("click",skip)

   document.body.appendChild(skiptitorial)
...
let barcolorchangeend=false

function interval() {
   

  if (speed>0.9) {
   if (barcolorchangeend) {
      let cho=Math.floor(Math.random()*2)
      spown=1800/(2*speed)
      clearInterval(yo)
      yo = setInterval(interval,spown)
